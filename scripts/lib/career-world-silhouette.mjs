...
function estimateBorderLuminance(pixels, width, height, channels) {
  const samples = [];
  ...
  for (let y = 0; y < height; y += stride) {
    for (let x = 0; x < width; x += stride) {
      ...
      const green = channels > 1 ? pixels[offset + 1] : red;
      const blue = channels > 2 ? pixels[offset + 2] : red;
...
function smoothEnvelope(values, maximum, windowRadius = 3) {
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    if (neighbors.length >= 2) {
      result[index] = Math.max(0, Math.min(maximum, median(neighbors)));
    }
  }
  return result;
}
...
export function buildForegroundMask(pixels, width, height, channels) {
  const background = estimateBorderLuminance(pixels, width, height, channels);
  ...
  for (let y = marginY; y < height - marginY; y += 1) {
    for (let x = marginX; x < width - marginX; x += 1) {
      ...
      const green = channels > 1 ? pixels[offset + 1] : red;
      const blue = channels > 2 ? pixels[offset + 2] : red;
      mask[pixelIndex] = luminance(red, green, blue) >= threshold ? 1 : 0;
    }
  }
  ...
  for (let index = 0; index < mask.length; index += 1) {
    alpha[index] = mask[index] ? 255 : 0;
  }
  return { alpha, background, threshold };
}

export function alphaBounds(alpha, width, height) {
  let left = width;
  let top = height;
  let right = -1;
  let bottom = -1;
  ...
  if (right < left || bottom < top) {
    throw new Error("Foreground mask is empty; refusing to emit a sprite");
  }
  return {
    left,
    top,
...
export function tightNormalizedRgba(
  pixels,
  alpha,
  sourceWidth,
  channels,
  ...
  for (let y = 0; y < bounds.height; y += 1) {
    ...
    for (let x = 0; x < bounds.width; x += 1) {
      ...
      const green = channels > 1 ? pixels[sourceOffset + 1] : red;
      const blue = channels > 2 ? pixels[sourceOffset + 2] : red;
      const gray = Math.max(0, Math.min(255, Math.round(luminance(red, green, blue) * brightness)));
      const destinationOffset =
        ((y + TRANSPARENT_GUARD_PX) * width + x + TRANSPARENT_GUARD_PX) * 4;
  ...
  return { data: rgba, width, height, guard: TRANSPARENT_GUARD_PX };
}
