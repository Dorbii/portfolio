...
async function build() {
  ...
  const atlasEntries = groups.flatMap((group) => [
    Object.freeze({
  ...
  const atlasName = "ninjaone-native-conifer-pool-atlas-r5.png";
  ...
  const atlasResource = Object.freeze({
    ...
    id: "native-conifer-pool-atlas",
    kind: "paired-foliage-atlas",
    path: versionedPublicPath(atlasPath, atlasDigest),
    sha256: atlasDigest,
    sourceMasterId: MASTER_SOURCE_ID,
  });
  const instances = groups.map((group) => Object.freeze({
  ...
  const manifest = {
    schemaVersion: 4,
    id: "career-world/capitals/ninjaone/foliage-native@r5",
    status: "active-r8-native-conifer-pool-with-neutralization",
    sourceMaster: {
    ...
    ownership: "L2_2 owns the registered conifer alpha, paired neutralization frames, pooled atlas, viewport admission, and root-pivoted animation; every canopy frame uses active-r8 RGB and every at-rest pair reproduces the authority plate exactly",
    quality: {
      atRestChangedPixels,
      minimumRegisteredConifers: 120,
      registeredConifers: instances.length,
      totalCanopyPixels,
  ...
  process.stdout.write(
    `Built ${instances.length} registered conifers in one ${ATLAS_WIDTH}x${atlasPlacement.height} atlas (${mountedFoliageNodes} maximum mounted nodes; ${combinedDecodedBytes} conservative decoded bytes).\n`,
  );
}
