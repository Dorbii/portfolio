...
void main() {
  ...
  if(uDetailPass>0.5) {
    float detailAlpha=oceanDetailAlpha*(1.0-inland);
    vec4 contact=coastContact(p,shore,shoreNormal,uTime.x,uWeather,geographyCoast);
    contact.a*=1.0-inland;
    float combined=contact.a+detailAlpha*(1.0-contact.a);
    color=(contact.rgb*contact.a+color*detailAlpha*(1.0-contact.a))/max(0.0001,combined);
    alpha*=combined;
  }
