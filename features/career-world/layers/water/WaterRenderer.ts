...
export class WaterRenderer {
  ...
  constructor(canvas: HTMLCanvasElement, scene: WaterScene, invalidate: () => void, detailCanvas?: HTMLCanvasElement) {
    ...
    canvas.dataset.surfaceRevision = "ocean-seabed-baked-layout-r2";
  ...
  render(oceanSeconds: number, inlandSeconds: number, events: readonly WaveEvent[] = []) {
    ...
    if (this.detailCanvas && this.detailContext) {
      ...
      if (scene.oceanVisible && scene.oceanDetailsVisible !== false && seabedState.detailsReady && !scene.debug) {
    ...
    canvas.dataset.drawCalls = String(1 + simulationPasses + Number(Boolean(this.detailContext) && scene.oceanDetailsVisible !== false && seabedState.detailsReady && scene.oceanVisible));
