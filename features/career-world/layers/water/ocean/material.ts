...
vec4 waveSurface(vec2 p, float time, float weather) {
  if (uSpectral>0.5) {
    ...
    return vec4(swell.rgb+ripple.rgb+distantSwell(q,time,weather),max(swell.a,ripple.a*0.35));
  }
...
vec3 oceanMaterial(vec2 p,float shore,vec2 shoreNormal,float time,float weather,float geographyCoast) {
  ...
  float surfRange=5.0+weather*5.0;
  ...
  vec3 attenuation=exp(-vec3(0.38,0.12,0.075)*(1.0+weather*0.5)*depth);
