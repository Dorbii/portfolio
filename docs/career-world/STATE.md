...
**T10b-runtime-polish IN FLIGHT** (terra · high, zero ImageGen) — check `.codex-tmp/qa/T9/t10b-runtime-run.log` / QA-REVIEW `**Codex (T10b-runtime-polish):**`. Ships against mounted canon r1: zoom floor 0.055→0.0825 (owner-ruled), tier-serving threshold fix (capital tier was serving at close range), shimmer masonry fix. After it: director F20 + full suite + commit.

**THEN dispatch T12-naturalize-then-tile (owner-approved "I'm a perfectionist, I agree", ~14 calls, sol):** the r2/r3 tile saga ended at three failures in three directions (F25/F26 + the naturalization principle: the generator can hold structure OR invent detail at tile scale, and synthetic scaffold elements cause recomposition drift — but PLATE-level generation naturalizes synthetic inputs, proven by anchor-and-cover). Plan: (1) regenerate the base plate from a scaffold carrying sprites + drawn paths + terrace pads (1–2 calls, T4d-r3 lane shape) so paths/terraces become native art; (2) full uniform re-tile of the canon from that improved plate with the EXACT T9b recipe (plain crops, single authority, no synthetic residue, no exemplar image refs — text only); (3) hero composite, stitch (40px), tiers, full gate suite (register continuity, vegetation floors vs r1, seating checklist, traced-eye circulation — the classifier gate is BROKEN on canon palettes, do not trust it); (4) owner review with overlays, then a mount lane swaps r4 in. All tile-iteration on r2/r3 is DEAD — three strikes, do not re-roll individual tiles again.
