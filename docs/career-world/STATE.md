...
**T11-r3-calibrated** (sol · high, ≤6 calls, owner-approved FINAL tile iteration — if it fails, STOP and reconvene on approach, no more calls) — background `codex exec`; check `.codex-tmp/qa/T9/canon-r3-final-run.log` and QA-REVIEW.md for `**Codex (T11-r3-calibrated):**`. Context: r2-continuation tiles were structurally perfect (drift ~0) but impoverished — Steve's live evidence: dark/gloomy band, no paths, buildings "on an incline, not part of the same image." The calibrated brief splits hold-exactly (structure/paths/coast) from must-flourish (vegetation to r1 density with a mechanical coverage floor, level terraced seating under every structure, register continuity ≤6% luminance delta at banked-tile borders). Note for a fresh thread: the stitched-circulation classifier gate is BROKEN on r3's palette (over-detects warm stone — see QA record); judge circulation by traced eye + the drawn-network overlay, and recalibrate the classifier before the next district.

Superseded r2-continuation record: Context: the first T11 pass failed honestly at 14/14 — its four multi-reference tiles drifted (→ **F26: one image authority per call; exemplars/style go in TEXT**), causing the broken borders/orphans/ghost bands; its five single-authority tiles were clean and are banked. The continuation retries tiles 03/06/07/08 with the enriched scaffold as sole image reference, re-stitches with the banked five, and re-runs the full stitched gate suite (circulation, borders, ghosts, exemplar eye pass). Output quarantines as `d05-canon-r3` FINAL under `.codex-tmp/quarantine/city-v2/T11-canon-r3/` with artifacts in `.codex-tmp/qa/T11r2/`.
