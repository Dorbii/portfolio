...
## 6.4 Register and animation direction (Steve, 2026-08-22 — supersedes the director's "moody-but-legible" grade decision)

The portfolio's target register is **bright and inviting, not moody**: warm late-day mountain light, vibrant forest greens, readable buildings at first glance — a recruiter's first impression should invite exploration. The master plate remains the authority for FORMS and vocabulary (architecture, materials, accents), NOT for its dark value floor. Skill buildings should **pop**: strong silhouettes, accent color, lit features. Design-ahead note: skill buildings will eventually be **animated** (turning wheels, chimney smoke, glowing windows, forge light) — render named buildings with clearly animatable features in animation-friendly poses. Open consistency item: the capital master's dusk register will eventually need reconciliation with the new world register.

## 6.5 Composition stack and in-context generation (Steve + director, 2026-08-22 — supersedes the standalone-sprite/no-terrain rules where they conflict)

Per-district composition stack: (1) native terrain (frozen L2/L3 backdrop); (2) **city-owned transition layer** — earthworks, terrace carving, urbanized ground, and city foliage additions that blend structures into the mountain; (3) structures; (4) props/detail.

Generation moves to **in-context**: assets are generated over the actual target terrain backdrop (the model paints into the context, matching scale/perspective/light/style by construction) and extracted by **difference against the known background** with ROI/changed-pixel gates (the original pipeline's method).

**Discard-don't-repair (Steve, 2026-08-22):** a candidate that violates its ROI — or fails any structural gate — is thrown away and regenerated fresh. No repair lanes, no salvage surgery, no partial rescues of bad generations; rework loops cost more than wasted calls. Deterministic post-fixes remain allowed only for otherwise-accepted assets with one bounded, named defect (the S10-tone-down class), never as a rescue path. The integration halo around each asset — repainted foundations, hugging trees, path aprons — is kept as transition-layer content. Fabric generates as neighborhood cluster patches, not single houses. Named buildings remain canonical: generated once in the capital context; project-city versions are repainted into new context with the canonical sprite as the identity reference. Chroma-keyed standalone generation remains valid only for context-free parts (props, circulation modules).
