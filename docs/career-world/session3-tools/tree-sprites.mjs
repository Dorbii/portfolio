...
// the crown classifier, as sway-field.mjs (dark saturated green with needle texture, closed)
function crownMask(d) {
  ...
  return ero(dil(keep, 2), 2);
}

// connected components of the closed mask, each with its box; small ones dropped
function components(closed) {
  ...
  for (let p = 0; p < W * W; p += 1) {
    ...
    comps.push({ id: cid, members, minx, maxx, miny, maxy, w: maxx - minx + 1, h: maxy - miny + 1, n: members.length });
  }
  return { lab, comps };
}
...
for (const t of ["ninjaone", "tanium", "coast"]) {
  ...
  for (const id of Object.keys(ledger.cells).sort()) {
    ...
    const d = img.data;
    const closed = crownMask(d);
    const { lab, comps } = components(closed);
    ...
    for (const c of singles) {
      ...
      // the foot: the crown's lowest rows, their mean x
      let fx = 0, fn = 0;
      for (const q of c.members) { const x = q % W, y = (q - x) / W; if (y >= c.maxy - 2) { fx += x; fn += 1; } }
      trees.push({
        box: [l, tp, w, h], foot: [Math.round(fx / fn), c.maxy + 1], height: c.h,
