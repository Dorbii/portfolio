...
sharp.cache(false);
const GEN = 2560, BLEED = 256, CELL = 2048, RUN = 96, TOL = 12 * Math.PI / 180, MAXWALK = 200, MAG = +(process.env.MAG ?? 30), GAP = 2;
...
for (let y = BLEED; y < BLEED + CELL; y += 2) for (let x = BLEED; x < BLEED + CELL; x += 2) { const i = y * GEN + x; if (!strong[i]) continue; n++;
  ...
  sumRun += run; if (run >= RUN) long++; hist[Math.min(7, Math.floor(run / 32))]++; }
console.log(`${(label ?? file).padEnd(26)} strong rock edges ${String(n).padStart(7)}  long-run(>=${RUN}px) ${(100 * long / (n || 1)).toFixed(1).padStart(5)}%  mean run ${(sumRun / (n || 1)).toFixed(1).padStart(5)} px  run hist(32px bins) ${Array.from(hist).map((v) => (100 * v / (n || 1)).toFixed(0)).join("/")}`);
