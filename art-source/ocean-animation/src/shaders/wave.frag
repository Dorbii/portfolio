...
// which this deliberately does NOT gate. Offline pins 1.
...
// crests are short. hForm below never got them: it was three cosines of the one
...
// not the spatially-varying wander/bend that this file records as failures at
// uDirWander and uDirBend. The distinction is the whole point: a fixed rotation
...
void main()
{
    ...
    // parallel bands straight onto a screenshot, and they are this term: an
    ...
    // Scrolled along the wave direction so a patch belongs to the water
    ...
    // uOpenWaveVis is 1 for the fixed plate this was tuned on, and nothing below
    ...
    // narrow fan, and the finer harmonic gets a much wider one, because short
    // waves are always more short-crested than the swell they ride on.
    //
    ...
    // every term downstream takes a gradient or a contour of this field, so the
