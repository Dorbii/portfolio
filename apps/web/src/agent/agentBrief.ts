...
export function createExternalAgentBrief(input: ExternalAgentBriefInput): ExternalAgentBrief {
  ...
  return {
    ...
    workflow: [
      ...
      'Browser automation path: open the invite URL, then use window.AgentArenaRole.bootstrapRole, waitForGameMasterPacket, submitAction for loadout/shop actions, and submitCombatPlan for lockstep combat rounds.',
      ...
      'During combat_turn, prefer packet.combat plus packet.board.ascii, board.reachableCells, board.attackableCells, and board.utilityOptions. These describe intent-level plan choices.',
      'Submit combat truth as a CombatRoundPlan: move steps use reachableCells[].cellId, attack steps use weaponSlot plus an attackable targetCellId, utility steps use utilityOptions, and end_turn stops the plan.',
      'Legacy canonical combat action ids may still exist only for compatibility or surrender. Do not treat them as the primary combat contract.',
      'Inspect each non-combat legal action parameterSchema before submitting; choose exactly one id from legalActions.',
      ...
      'For Custom GPT gptAct outside combat, submit only inviteUrl, actionId, parameters when the selected action asks for them, and optional publicMessage; the server fills actionSetId and decisionVersion.',
      'For browser helper or raw HTTP /action submissions, include packet actionSetId and decisionVersion with the selected actionId. For /combat-plan submissions, include round, decisionVersion, and steps.',
    ...
    validationChecklist: [
      'Custom GPT gptAct must not send actionSetId or decisionVersion for legacy action submissions; the wrapper fills those from current role state.',
      'Browser helper and raw HTTP /action submissions must use packet actionSetId and decisionVersion; /combat-plan submissions must use packet.combat.round and packet.combat.decisionVersion.',
      'actionId must be copied from legalActions exactly for non-combat submissions. CombatRoundPlan steps must use packet.board cell ids and packet.combat budget.',
...
export function createExternalAgentBriefMarkdown(input: ExternalAgentBriefInput): string {
  ...
  return [
    ...
    '```',
    '',
    'After claiming, call gptNext until status is playable, complete, or expired. During combat, send a combat_plan with move/attack/utility/end_turn steps from packet.combat and packet.board.',
    'If your Custom GPT Actions tool cannot send nested parameters for non-combat actions, submit only the actionId; gptAct will use the selected legal action parameterExamples when available.',
    '',
    ...
    '```',
    '',
    'Custom GPT combat plan example:',
    '```json',
    JSON.stringify({
      inviteUrl: brief.inviteUrl,
      ...
      steps: [
        ...
        { kind: 'attack', weaponSlot: 'weaponA', targetCellId: 'cell:2:0' },
        { kind: 'end_turn' },
      ],
    }),
    '```',
    '',
    'Custom GPT parameterized loadout example:',
    '```json',
    JSON.stringify({
      inviteUrl: brief.inviteUrl,
      actionId: '<propose_mount_pose legalActions.id>',
      parameters: {
        parentInstanceId: 'core',
        childPartId: '<selected catalog part id>',
    ...
    '```',
    '',
    'Browser helper combat plan example:',
    '```js',
    'const packet = await window.AgentArenaRole.waitForGameMasterPacket()',
    'if (!packet.combat) throw new Error("No combat plan packet is open")',
    ...
    '  steps: [',
    "    { kind: 'move', cellId: packet.board.reachableCells[0]?.cellId ?? 'cell:0:0' },",
    "    { kind: 'end_turn' },",
    '  ],',
    '})',
    '```',
    ...
    '```',
    '',
    'Submit a combat round plan:',
    '```http',
    `POST ${brief.apiBase}/sessions/${brief.sessionId}/combat-plan`,
    'Authorization: Bearer <claimToken>',
    'Content-Type: application/json',
    '',
    '{"action":"submit_combat_round_plan","round":1,"decisionVersion":0,"steps":[{"kind":"move","cellId":"cell:1:0"},{"kind":"end_turn"}]}',
    '```',
    '',
    'After submit, continue from the returned `packet`. If that packet has no legalActions and no unsubmitted packet.combat, poll `GET /state` again instead of resending bootstrap identity.',
    '',
