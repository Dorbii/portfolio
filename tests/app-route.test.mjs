...
test('root console is wired to live referee session helpers', () => {
  ...
  assert.ok(refereePanelsSource.includes('Copy invite'))
  assert.ok(refereePanelsSource.includes('Copy handoff'))
  assert.ok(refereeConsoleSource.includes('inviteCopyUrl: redInviteUrl'))
  assert.ok(refereeConsoleSource.includes('inviteCopyUrl: blueInviteUrl'))
  assert.ok(refereeConsoleControllerSource.includes('copyInviteUrl'))
...
test('agent cockpit renders reliability and debug hooks', () => {
  ...
  assert.equal(cockpitRuntimeSource.includes('AgentTaskPanel'), false)
  assert.equal(cockpitRuntimeSource.includes('ConnectionGuide'), false)
  assert.equal(cockpitRuntimeSource.includes('connection-heading'), false)
  assert.equal(cockpitRuntimeSource.includes('secondary-state-heading'), false)
  assert.ok(cockpitRuntimeSource.includes('Public Chat'))
  assert.equal(agentInsightSource.includes('Assembly bay'), false)
  ...
  assert.ok(botAssemblySceneSource.includes('data-assembly-visual-authority={visualAuthority}'))
  ...
  assert.ok(cockpitRuntimeSource.includes('refreshButtonLabel'))
  assert.equal(cockpitRuntimeSource.includes('claimButtonLabel'), false)
...
test('agent cockpit is a read-only insight surface instead of a bot editor', () => {
  ...
  assert.ok(agentInsightSource.includes('Build snapshot'))
  assert.ok(agentInsightSource.includes('Build details'))
  assert.ok(agentInsightSource.includes('WorkbenchDetailDrawer'))
  assert.ok(agentInsightSource.includes('Equipped parts'))
  assert.ok(agentInsightSource.includes('Current equipped context'))
  assert.ok(agentInsightSource.includes('Store offers'))
  assert.ok(agentInsightSource.includes('Owned parts'))
  assert.ok(agentInsightSource.includes('NoRoleStatePanel'))
  assert.ok(agentInsightSource.includes('roleState?.ownLoadout'))
  assert.ok(agentInsightSource.includes('StoreReadoutTab'))
  assert.ok(agentInsightSource.includes('role="tablist"'))
  assert.ok(agentInsightSource.includes('role="tabpanel"'))
  assert.ok(agentInsightSource.includes('gameMaster?.build'))
  assert.ok(agentInsightSource.includes('CatalogPartList'))
  assert.ok(agentInsightSource.includes('gameMaster?.store'))
  assert.ok(agentInsightSource.includes('gameMaster?.legalActions'))
  assert.ok(agentInsightSource.includes('state?.inventory'))
  assert.equal(agentInsightSource.includes('Combat decision'), false)
  assert.equal(agentInsightSource.includes('tacticalCues'), false)
  assert.equal(agentInsightSource.includes('Action packet'), false)
  assert.equal(agentInsightSource.includes('PacketActionSummary'), false)
  assert.equal(agentInsightSource.includes('createBoardActionSummary'), false)
  assert.equal(agentInsightSource.includes('Machine legality'), false)
  assert.equal(agentInsightSource.includes('formatLegalAction'), false)
  assert.equal(agentInsightSource.includes('inspect parameterSchema and include parameters'), false)
...
test('agent cockpit omits redundant task and state sidebars', () => {
  const cockpitSurfaceSource = [
    cockpitSource,
    cockpitShellSource,
    cockpitSidebarSource,
  ].join('\n')
  assert.equal(cockpitSurfaceSource.includes('AgentTaskPanel'), false)
  assert.equal(cockpitSurfaceSource.includes('agent-task-panel'), false)
  assert.equal(cockpitSurfaceSource.includes('secondary-state-heading'), false)
  assert.equal(cockpitSurfaceSource.includes('connection-heading'), false)
  assert.equal(cockpitSurfaceSource.includes('ConnectionGuide'), false)
  assert.equal(cockpitViewStateSource.includes('createAgentCockpitWorkflow'), false)
  assert.ok(cockpitSource.includes('cockpit-primary-column'))
  assert.ok(cockpitSurfaceSource.includes('cockpit-secondary-stack'))
  assert.ok(cockpitSource.includes('showSecondarySidebar'))
  assert.ok(cockpitSource.indexOf('<AgentCockpitHeader') < cockpitSource.indexOf('<AgentInsightWorkbench'))
})
...
test('Agent Arena operational surfaces use shared UI primitives', () => {
  ...
  assert.ok(cockpitSurfaceSource.includes('Button'))
...
test('agent cockpit exposes submitted truth without editable draft state', () => {
  ...
  assert.ok(botAssemblySceneSource.includes('data-assembly-visual-authority={visualAuthority}'))
  assert.ok(botAssemblySceneSource.includes("return 'empty-garage'"))
  assert.ok(botAssemblySceneSource.includes('clearAssemblyBot(resources)'))
  ...
  assert.ok(agentInsightSource.includes('StoreReadoutTab'))
  assert.ok(agentInsightSource.includes('NoRoleStatePanel'))
  assert.equal(agentInsightSource.includes('AssemblyBayPlaceholder'), false)
  assert.equal(agentInsightSource.includes('No confirmed loadout is available yet.'), false)
...
test('referee dashboard embeds observer cockpits with machine-authority garage renders', () => {
  ...
  assert.ok(refereeConsoleSource.includes('placement="stage"'))
  assert.ok(refereeConsoleSource.includes('!shouldShowReplay'))
  assert.ok(refereeConsoleSource.includes("visibility: 'role_only'"))
  assert.ok(refereeConsoleSource.includes('Agent comms'))
  assert.ok(refereeCockpitStripSource.includes('roleState?.privateChatLog ?? []'))
  assert.ok(refereeCockpitStripSource.includes('aria-label="Agent role-only decision notes"'))
  assert.ok(refereeCockpitStripSource.includes('arena-monologue-card'))
  assert.equal(refereeConsoleSource.includes('RefereeArenaMonologueOverlay'), false)
  assert.equal(refereeCockpitStripSource.includes('No tactical cues.'), false)
  assert.equal(refereeCockpitStripSource.includes('No legal actions in packet.'), false)
